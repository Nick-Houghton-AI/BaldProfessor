...
[LM Studio](https://lmstudio.ai) works just as well, and you can have both.

Open **Settings**, *AI model*. Bald Professor finds whichever servers are running
and lists **every model each one actually has**, with a search box, so you pick
from a real list rather than typing a name and hoping. Click one and it switches,
including across from Ollama to LM Studio or back. You never have to know about
ports or config files.

If the model in your settings is not on the server you are pointed at, it says so
on that screen rather than letting the next generation fail. That mismatch is easy
to hit, because an Ollama tag like `gemma3:4b` means nothing to LM Studio.
